...
%
% do this:
% {ok, LinkedPid} = libcouch_event_sup:start_link(error_logger, my_log, Args)
%
...
% {ok, LinkedPid} = libcouch_event_sup:start_link({local, my_log}, error_logger, my_log, Args)
%

start_link(EventMgr, EventHandler, Args) ->
    gen_server:start_link(libcouch_event_sup, {EventMgr, EventHandler, Args}, []).

start_link(ServerName, EventMgr, EventHandler, Args) ->
    gen_server:start_link(ServerName, libcouch_event_sup, {EventMgr, EventHandler, Args}, []).
