...
prep_updates(_Db, [], [], _AllowConflict, AccPrepped,
        AccFatalErrors) ->
   {AccPrepped, AccFatalErrors};
prep_updates(Db, [DocBucket|RestBuckets], [not_found|RestLookups],
    ...
    prep_updates(Db, RestBuckets, RestLookups, AllowConflict,
...
has_atts(Docs) ->
    lists:any(fun({#doc{atts=Atts}, _Ref}) ->
                Atts /= []
        end, Docs).
...
update_docs(Db, Docs, Options, replicated_changes) ->
    ...
    {DocErrors, DocBuckets3} = case has_atts(Docs2) of
        true ->
            Ids = [Id || [{#doc{id=Id}, _Ref}|_] <- DocBuckets],
            ExistingDocs = get_full_doc_infos(Db, Ids),

            {DocBuckets1, DocErrors1} =
                    prep_and_validate_replicated_updates(Db, DocBuckets,
                                                         ExistingDocs, [],
                                                         []),
            % remove empty buckets
            DocBuckets2 = [Bucket || [_|_]=Bucket <- DocBuckets2],
            {DocErrors1, DocBuckets2};
        false ->
                {DocErrors, DocBuckets}
    end,
...
update_docs(Db, Docs, Options, interactive_edit) ->
    ...
    {PreCommitFailures, DocBuckets2} = case has_atts(Docs3) of
        true ->
            % lookup the doc by id and get the most recent
            Ids = [Id || [{#doc{id=Id}, _Ref}|_] <- DocBuckets],
            ExistingDocInfos = get_full_doc_infos(Db, Ids),

            {DocBucketsPrepped, PreCommitFailures1} = prep_updates(Db,
